...
class IDataHandler(ABC):
    ...
    def ohlcv_data_min_max(
        ...
        :param pair: 要获取最小/最大时间的交易对
        :param timeframe: 要获取最小/最大时间的时间框架
        :param candle_type: 枚举CandleType中的任何一种（必须与交易模式匹配！）
        :return: (最小时间, 最大时间, 长度)
        """
        df = self._ohlcv_load(pair, timeframe, None, candle_type)
        if df.height == 0:  # Polars使用height属性检查空数据
            return (
                datetime.fromtimestamp(0, tz=timezone.utc),
                datetime.fromtimestamp(0, tz=timezone.utc),
                0,
            )
        # 使用Polars的高效方法获取第一行和最后一行的date列
        return (
            df.select(pl.col("date").first()).item(),
            df.select(pl.col("date").last()).item(),
            df.height  # Polars使用height属性获取行数
        )
    ...
    def trades_data_min_max(
        ...
        return (
            df.select(pl.col("date").first()).item().to_pydatetime(),
            df.select(pl.col("date").last()).item().to_pydatetime(),
            len(df),
        )
    ...
    def trades_store(self, pair: str, data: DataFrame, trading_mode: TradingMode) -> None:
        ...
        self._trades_store(pair, data[DEFAULT_DATAFRAME_COLUMNS], trading_mode)
